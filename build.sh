#!/bin/bash
sudo apt update && sudo apt install ccache bc bash git-core gnupg build-essential zip curl make automake autogen autoconf autotools-dev libtool shtool python m4 gcc libtool zlib1g-dev libstdc++6
# Export
...
#export clang_repo="https://github.com/NusantaraDevs/clang.git"
#export clang_branch="dev/10.0"
#export clang_triple="aarch64-linux-gnu-"
export zip_name="wulan17-kernel-""$device""-"$(env TZ='Asia/Jakarta' date +%Y%m%d)""
export KERNEL_DIR=$(pwd)
export KERN_IMG="$KERNEL_DIR"/out/arch/"$ARCH"/boot/Image.gz-dtb
...
function sync(){
	...
	cd "$KERNEL_DIR" && git clone --quiet "$THREAD" https://github.com/wulan17/AnyKernel3 AnyKernel
	#cd "$KERNEL_DIR" && git clone --quiet "$THREAD" -b "$clang_branch" "$clang_repo" --depth 1 clang > /dev/null
	#wget -q "$clang_url"
	#mkdir -p clang
